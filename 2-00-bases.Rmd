...
# (PART) II. BASES DE DADOS {-}

# Bases de dados. Dados do PPBio Semiárido {#bases}
...
Usaremos ao longo desse livro dados que fazem parte de um estudo mais amplo sobre ecologia de rios do semiárido, coletados no Programa de Pesquisa em Biodiversidade - PPBio (Veja [Programa de Pesquisa em Biodiversidade -- PPBio](https://ppbio.inpa.gov.br/Sobre)).

Nesta base de dados estão armazenadas informações sobre diversos grupos taxonômicos dstribuidos em diversas unidades amostrais (UA's ou sítios), como peixes, macroinverbrebrados bentônicos, quironomídeos e zooplâncotn, além de dados do habitat, como variáveis físicas e químicas, morfologia do habitat, composição do substrato, estrutura de habitat marginal, entre outros (veja a seção [Arquivos disponíveis](#arqs), a seguir).

Parte desses dados está armazenada em planilhas de Excel `ppbio**.xlsx` (Figura \@ref(fig:200ppbio)). Essas matrizes de dados são descritas na Tabela \@ref(tab:200arqs). As planilhas `ppbio**.xlsx` contém vários tipos de dados arranjados em matrizes `n x m` que incluem dados de abundância de espécies em diferentes unidades amostrais (UA's), dados da estrutura do habitat físico, e variáveis em escala de bacia hidrográfica, dados de contagem de indivíduos ajustados para Captura Por Unidade de Esforço (CPUE), etc (Figura \@ref(fig:200ppbio)).

```{r 200ppbio, echo=FALSE, purl=FALSE, fig.cap="Parte da planilha de dados brutos do PPBio.", out.width = if (knitr::is_html_output()) "500px" else "10cm", fig.asp=1}
knitr::include_graphics("imagens/ppbio.png")
```

Por exemplo, essa é a **matriz bruta de dados**, porque os valores ainda não foram ajustados para os valores de Captura Por Unidade de Esforço (CPUE), nem foram relativizados ou transformados. Outros tipos de arquivos existem sobre esses dados (Tabela \@ref(tab:200arqs)).

Várias das espécies nessa matriz tem grande importância ecológica, como é o caso de *Astyanax bimaculatus*^[A etimologia do gênero *Astyanax* vem da mitologia Grega. Heitor personagem da "Ilíada", tinha um filho chamado Astíanax.] (Figura \@ref(fig:200asbim)), que é muito comum em rios intermitentes e serve de alimento para predadores maiores [@RN2908] como a espécie *Hoplias malabaricus*^[Do Grego, *hoplon*, arma ou armadura, em referência aos dentes caniniformes muito desenvolvidos, e forte estrutura óssea na cabeça.] (Figura \@ref(fig:200hmala)) [@RN2724].

```{r 200asbim, echo=FALSE, purl=FALSE, fig.cap="*Astyanax bimaculatus*, a espécie mais comum da matriz de dados ppbio. Peru, by Eakins, R. Fonte: <https://www.fishbase.se/summary/Astianax-bimaculatus.html>", out.width = if (knitr::is_html_output()) "450px" else "10cm", fig.asp=1}
knitr::include_graphics("imagens/spp/Asbim_u4.jpg")
```

```{r 200hmala, echo=FALSE, purl=FALSE, fig.cap="*Hoplias malabaricus*, espécie que cresce para se tornar um importante predador. Brazil, by Roselet, F.F.G. Fonte: <https://www.fishbase.se/summary/Hoplias-malabaricus.html>", out.width = if (knitr::is_html_output()) "450px" else "10cm", fig.asp=1}
knitr::include_graphics("imagens/spp/Homal_u6.jpg")
```

Outras espécies como *Apareiodon hasemani*^[A etimologia do nome *Apareiodon* vem do Grego, *a*, sem, *pareia*, lateral ou bochecha, e *odous* dentição, em referência a ausência de dentes laterais no aparato bucal dessa espécie."] (Figura \@ref(fig:200apdavis) tem importância trófica por estar na base da cadeia alimentar, enquanto espécies da família Loricariidae, como *Pseudancistrus genisetiger*^[A etimologia do nome *Pseudancistrus* vem do Grego, *pseudes*, falso, e *agkistron*, gancho, em referência a falsos ganchos presentes na cabeça em algumas espécies do gênero.] (Figura \@ref(fig:200pgen)), tem importância para taxonomia.

```{r 200apdavis, echo=FALSE, purl=FALSE, fig.cap="*Apareiodon sp.*, importante espécie bentopelágica das bacias dos rios Jaguaribe e Paraíba. Brazil, by Ramos, T.P.A. Fonte: <https://www.fishbase.se/summary/Apareiodon-davisi.html>", out.width = if (knitr::is_html_output()) "450px" else "10cm", fig.asp=1}
knitr::include_graphics("imagens/spp/Apdav_u0.jpg")
```

```{r 200pgen, echo=FALSE, purl=FALSE, fig.cap="*Pseudancistrus genisetiger*, uma espécie endêmica das bacias hidrográfcas do nordeste. By Medeiros, E.S.F. Fonte: Arquivo pessoal", out.width = if (knitr::is_html_output()) "450px" else "10cm", fig.asp=1}
...
```{r 200del, echo=FALSE, purl=FALSE, fig.cap="Associação entre a planilha de dados brutos do PPBio e o delineamento amostral do estudo.", out.width = if (knitr::is_html_output()) "600px" else "13cm", fig.asp=1}
knitr::include_graphics("imagens/rm1.fig3.png")
```

## Arquivos disponíveis {#arqs}
...
```{r 200arqs, echo=FALSE, warning=FALSE, purl=FALSE}
library(tibble)
library(gt)
arqs_nada <- tribble(
  ~`Arquivo`, ~`Tipo de matriz`, ~Descrição, ~`Tipo de dados`,
  "[ppbio06c-peixes.xlsx](https://docs.google.com/spreadsheets/d/1Olyew6L3SCFeSXWJC54Ak3MKmgHD3sTK/edit?usp=drive_link&ouid=101766125969589673518&rtpof=true&sd=true); [ppbio06c-peixes.ods](https://docs.google.com/spreadsheets/d/1Olyew6L3SCFeSXWJC54Ak3MKmgHD3sTK/edit?usp=drive_link&ouid=101766125969589673518&rtpof=true&sd=true)",
  "Matriz comunitária",
  ...
  "[ppbio06cpue-peixes.xlsx](https://docs.google.com/spreadsheets/d/1hXSKaJlLhzMhfMUs1jXR1ynttcr4n14P/edit?usp=sharing&ouid=101766125969589673518&rtpof=true&sd=true)",
  "Matriz comunitária",
...
gt_tbl <- tab_caption(
  gt_tbl,
  caption = "Matrizes disponíveis para análises, com suas descrições e tipos de dados recomendados."
)
gt_tbl <- fmt_markdown(
  gt_tbl,
  columns = Arquivo)
...
```{r 200codes, echo=FALSE, purl=FALSE, fig.cap="Codificação para as variáveis, espécies de peixes, sítios de amostragem e período de amostragem", out.width = if (knitr::is_html_output()) "500px" else "10cm", fig.asp=1}
