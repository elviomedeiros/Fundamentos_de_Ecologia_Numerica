...
# (PART) VI. INFORMAÇÕES GERAIS {-}

# Nomenclatura das matrizes em AMD {#nomes} 

No tocante aos tipos de atributos e seu tratamento pré-análises, uma matriz de dados pode ser dos tipos apresentados na tabela abaixo (Tabela \@ref(tab:601tbl-m)).

```{r 601tbl-m, echo=FALSE, purl=FALSE}
m_ <- tibble::tribble(
...
## Grupos taxonômicos {#601taxon}

Vários grupos taxonômicos foram amostrados, além de variáveis ambientais, análise de conteúdo estomacal de peixes, estrutura de tamanho dos indivíduos e etc. Isso é estabelecido no nome do arquivo .xlsx fornecido, como apresentado na tabela abaixo (Tabela \@ref(tab:601taxon)). O \* se indica o tipo de matriz (Tabela \@ref(tab:601arqs))  

```{r 601taxon, echo=FALSE, purl=FALSE}
m_ <- tibble::tribble(
  ~"Nome", ~"Atributos (colunas)",
...
## Tipos de matrizes {#601arqs}

Matrizes disponíveis para análises, com suas descrições e tipos de dados recomendados (Tabela \@ref(tab:601arqs)).

```{r 601arqs, echo=FALSE, purl=FALSE}
arqs <- tibble::tribble(
...
## Abreviações {#601tblm2}

No interesse de sistematizar o código R das várias matrizes que são comumente usadas em uma AMD, a tabela a seguir (\@ref(tab:601tbl-m2)) resume seus tipos e abreviações.

```{r 601tbl-m2, echo=FALSE, purl=FALSE}
m <- tibble::tribble(
...
library(knitr)
library(kableExtra)
m_x <- kable(m, row.names = FALSE, align = c("l", "c", "c"), booktabs = TRUE, caption = "Nomenclatura das matrizes em AMD em relação aos atributos das colunas.")
m_x <- kable_styling(m_x)
column_spec(m_x, 1:3, width = c("2cm","5cm","2cm"))
```

## Bases de dados para download {#601links}

```{r 601links, echo=FALSE, purl=FALSE, message=FALSE, eval=FALSE}
library(dplyr)
library(kableExtra)
arqs <- tibble::tribble(
