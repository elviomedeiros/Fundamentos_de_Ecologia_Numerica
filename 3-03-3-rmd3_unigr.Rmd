...
# R Modulo 3.3 - Gráficos univariados {#unigr}
...
Considere os dados merísticos (ou médições morfológicas) da espécie de peixe *Cichla ocellaris* (tucunaré amarelo) do reservatório da barragem  de Gramame, PB [@RN3024] (Figura \@ref(fig:rmd32tucu)). Existem 421 medições do comprimemto total (CT), comprimento padrão (CP) e peso total (PT), além do sexo (MACHO, FÊMEA ou imaturo), e outros descritores da estrutura populacional da espécie, um conjunto de dados formidável. 

```{r rmd32tucu, echo=FALSE, purl=FALSE, fig.cap="Dados merísticos da espécie de peixe *Cichla ocellaris* (tucunaré amarelo) do reservatório da barragem  de Gramame, PB.", out.width = if (knitr::is_html_output()) "450px" else "10cm"}
...
::: {.center data-latex=""}
Ou, baixe aqui o arquivo [tucuna.xlsx](https://docs.google.com/spreadsheets/d/1CXcIx3vzNmVlaq87Mp_FLcmymLncb8P_/edit?usp=sharing&ouid=101766125969589673518&rtpof=true&sd=true)
:::
::::

Vamos importar a planilha de dados univariados `tucuna.xlsx`. Note que o símbolo [`#`]{style="color:green"} em programação R significa que o texto que vem depois dele é um comentário e não será executado pelo programa. Isso é útil para explicar o código ou deixar anotações. Ajuste a segunda linha do código abaixo para refletir ["C:/Seu/Diretório/De/Trabalho/Planilha.xlsx"]{style="color:blue"}.
...
https://www.geeksforgeeks.org/break-axis-of-plot-in-r/
https://stackoverflow.com/questions/72498597/add-a-break-to-a-bar-chart-when-one-value-is-very-large
...
knitr::purl(input = "3-03-3-rmd3_unigr.Rmd", output = "script-3-03-3-rmd3_unigr.txt", documentation = 0)
Script <- readLines("script-3-03-3-rmd3_unigr.txt")
Script <- Script[Script != ""]
writeLines(Script, "script-3-03-3-rmd3_unigr.txt")
cat(Script, sep = "\n")
```
